      ...
      <div class="col-xs-12">
        <div class="box box-primary">
          <div class="box-header with-border">
            <h3 class="box-title">待审核实名用户列表</h3>
          </div>
              ...
              <tbody>
                <template v-for="item in tableList">
                  <tr>
                    ...
                    <td>{{ item.phone }}</td>
                    <td>{{ item.idCard }}</td>
                    <td><button class="btn btn-default btn-xs" @click="viewPhoto(item.idCardPhoto)">查看</button></td>
                    <td>
...
<script>
   ...
   export default {
    data() {
        return {
          showEmptyTip: false,
          emptyTip: "暂时还没有实名用户申请",
          //分页相关
          lenArr: [10, 50, 100],
          pageLen: 5,
          param: {},
          url: commFun.apiRouter.realnameApplyList,
          tableList: []
        }
    },
    components: {
    ...
    methods: {
      viewPhoto: function(photo1){
        var self = this;
        var photoArr = [];
        photoArr.push({text:'', photo: photo1});
        commFun.info_tip({title:'身份证照片', imgArr: photoArr});
      },
      // 拒绝申请
